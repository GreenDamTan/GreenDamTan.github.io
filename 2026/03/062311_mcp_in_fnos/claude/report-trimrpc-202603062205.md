# libtrimrpc.so.0.1.3 反汇编级逆向分析报告

> **目标**: `libtrimrpc.so.0.1.3` (fnOS 1.1.15 trimrpc IPC 框架核心共享库)
> **架构**: x86_64 ELF (System V AMD64 ABI, little-endian)
> **工具**: IDA Pro 9.0 + ida-pro-mcp (只读分析, 未修改二进制)
> **日期**: 2026-03-06
> **函数总计**: 590 (trimrpc 命名空间 266)
...
所有服务通过中央 Broker (`com.trim.rpcbroker`, UDS `/var/run/rpc/rpcbroker`) 进行注册与发现。客户端先向 Broker 申请权限获取目标服务的 UDS 路径和服务类型, 随后直连目标服务进行 RPC 通信 — 这是服务发现模式, Broker 不参与后续数据转发。
...
1. **服务名验证缺陷** (0x256C0): 仅检查 `dword [rdx] == 0x2E6D6F63` 和 `size ≤ 128`。任何本地进程可构造 `com.` 前缀服务名注册到 Broker, 无 ACL 隔离。

2. **会话 ID 可预测** (0x14955, 0x12DC2): `lock xadd` 原子递增, 从 1 开始单调递增。攻击者可预测下一个 session_id 并抢先注入伪造响应。

3. **单次重试无退避** (0x193D8): `SendPacket` 失败后立即重试一次, 无指数退避, 无速率限制。

4. **固定超时** (0x13E88): 默认 10 秒, 通过 `SetTimeOut` 可调。无心跳机制, 连接静默断开时需等待完整超时周期。

5. **明文 UDS 通信**: 全部通过 Unix Domain Socket 明文传输。安全边界依赖于 `/var/run/rpc/` 目录的文件系统权限。

6. **Broker UDS 硬编码** (0x19611): `"com.trim.rpcbroker"` 路径在 `InitServiceMap` 中硬编码。若攻击者在 Broker 启动前创建同名 UDS 文件, 可实施中间人攻击。

---

**分析完成**。本报告基于对 `libtrimrpc.so.0.1.3` 二进制文件的纯静态逆向分析, 所有地址和指令序列均直接来自 IDA Pro 反汇编输出, 未进行任何二进制修改。
